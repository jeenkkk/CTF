from this site it like a game that we have to choose path to find the flag but we will skip that
let see the code
![alt text](image.png)
...
```
as we can see that we need secret in availableOptions at async function CheckMessage()
we found interesting api from this function const fetchOptions so go to it
![alt text](image-2.png)
all commands are leak so we need secret to get the flag by using burp suite
![alt text](image-3.png)
change the parameter to get the flag
